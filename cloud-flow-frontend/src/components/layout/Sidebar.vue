<template>
  <aside class="fixed left-0 top-0 bottom-0 w-64 bg-dark-800 border-r border-dark-600 flex flex-col z-40">
    ...
    <nav class="flex-1 p-4 overflow-y-auto">
      <div class="mb-4">
        <p class="text-xs font-semibold text-gray-500 uppercase tracking-wider px-2 mb-2">CloudFlow</p>
        <ul class="space-y-1">
      ...
      <div>
        <p class="text-xs font-semibold text-gray-500 uppercase tracking-wider px-2 mb-2">External Tools</p>
        <ul class="space-y-1">
          <li v-for="item in externalTools" :key="item.id">
            <button
              @click="$emit('change', item.id)"
              :class="[
                'w-full flex items-center gap-3 px-4 py-2.5 rounded-lg transition-all duration-200 text-left',
...
const externalTools = [
  ...
  { id: 'jaeger', label: 'Jaeger', icon: Search },
  { id: 'clickhouse', label: 'ClickHouse', icon: Database },
  { id: 'alertmanager', label: 'Alertmanager', icon: AlertTriangle }
]
</script>
