  ...
  <div class="space-y-6">
    <div class="flex items-center justify-between">
      <div>
        ...
        <p class="text-sm text-slate-500 dark:text-slate-400 mt-1">Namespace级别的依赖关系</p>
      </div>
    ...
    <div class="grid grid-cols-4 gap-4 mb-4">
      <div class="card p-4">
        <p class="text-sm text-slate-500">Namespace数</p>
      ...
      <div class="card p-4">
        <p class="text-sm text-slate-500">异常节点</p>
        <p class="text-2xl font-bold text-red-500 mt-1">
          <Loader2 v-if="loading" class="w-5 h-5 animate-spin inline" />
          <span v-else>{{ errorCount }}</span>
        </p>
      </div>
    </div>
    ...
    <div class="card p-6">
      <div class="h-[500px] relative">
        ...
        <ECharts v-else :option="topologyOption" class="w-full h-full" autoresize />
      </div>
    </div>
  </div>
</template>
...
const healthyCount = computed(() => nodes.value.filter((n) => !n.status || n.status === 'healthy' || n.status === 'ok' || n.status === 'normal').length)
const errorCount = computed(() => nodes.value.filter((n) => n.status === 'error' || n.status === 'critical' || n.status === 'offline' || n.status === 'warning').length)
...
const topologyOption = computed(() => {
  const chartNodes = nodes.value.map((n) => ({
    ...
    symbolSize: n.size || 35,
    category: n.group || n.cluster || 'default',
  }))
  ...
  const chartLinks = links.value.map((l) => ({
    ...
    lineStyle: {
      width: l.value ? Math.min(5, Math.max(1.5, l.value / 10)) : 2,
  ...
  return {
    ...
    series: [{
      ...
      links: chartLinks,
      categories,
      lineStyle: { width: 2, color: '#cbd5e1', curveness: 0.1, opacity: 0.7 },
...
const fetchData = async () => {
  loading.value = true
  try {
    const res = await queryService.getTopology({ type: 'namespace' }).catch(() => null)
