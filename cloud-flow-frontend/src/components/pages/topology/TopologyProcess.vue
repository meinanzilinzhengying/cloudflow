  ...
  <div class="space-y-6">
    <div class="flex items-center justify-between">
      <div>
        ...
        <p class="text-sm text-slate-500 dark:text-slate-400 mt-1">进程级别的依赖关系</p>
      </div>
...
const topologyOption = computed(() => {
  const chartNodes = nodes.value.map((n) => ({
    ...
    category: n.host || n.group || n.namespace || 'default',
  }))
...
const fetchData = async () => {
  loading.value = true
  try {
    const res = await queryService.getTopology({ type: 'process' }).catch(() => null)
