<template>
  <aside class="fixed left-0 top-0 bottom-0 w-64 bg-dark-800 border-r border-dark-600 flex flex-col z-40">
    <div class="p-4 border-b border-dark-600">
      <div class="flex items-center gap-3">
        ...
        <div>
          ...
          <p class="text-xs text-gray-400">业务监控</p>
        </div>
      </div>
    </div>
    ...
    <nav class="flex-1 p-4 overflow-y-auto">
      <div class="mb-4">
        <p class="text-xs font-semibold text-gray-500 uppercase tracking-wider px-2 mb-2">业务模块</p>
        <ul class="space-y-1">
          <li v-for="item in enabledMenuItems" :key="item.id">
            <button
...
const enabledMenuItems = computed(() => {
  return allMenuItems.filter(item => {
    const featureKey = item.id === 'alerts' ? 'alerts' : item.id
    return features.modules[featureKey]?.enabled !== false
  })
})
</script>
